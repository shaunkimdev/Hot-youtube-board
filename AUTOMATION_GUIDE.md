...
요약 품질을 자동으로 유지하려면 ③단계에 **LLM API**(예: Claude API)를 연결해야 합니다:
- 각 영상의 제목+설명+자막을 Claude API에 보내 100자 요약 생성 → 비용은 영상당 극히 저렴(텍스트만).
- `/watch` 프레임 분석까지 무인 자동화하려면 yt-dlp 다운로드 + 프레임 + 멀티모달 API 호출이 필요(영상당 비용·시간 큼) → **상위 N개만** 선별 적용 권장.
...
- **요약 품질 업그레이드(권장)**: `build_auto.py`의 `auto_summary()`를 Claude API 호출로 교체하면 무인 상태에서도 100자 요약 품질 유지. (제목+설명+자막 → 요약 프롬프트)
